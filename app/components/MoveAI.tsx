...
const COLORS = {
  bg: "#050814", bgCard: "rgba(255,255,255,0.04)", bgCardHover: "rgba(255,255,255,0.07)",
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.text, fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px 2rem 4rem", textAlign: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800 }}>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2.5rem, 6vw, 4.5rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-2px", marginBottom: 8 }}>Relocate & grow your career</h1>
      ...
      <section style={{ padding: "6rem 2rem", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>
          {[
            ...
            { icon: "💼", title: "Real Job Listings", desc: "Live job offers pulled in real time from Adzuna for your profession in each recommended city." },
      ...
      <section style={{ padding: "6rem 2rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, letterSpacing: "-1px", marginBottom: 20 }}>Your next chapter starts here.</h2>
...
function JobListings({ city, profession }: { city: CityData; profession: string }) {
  ...
  useEffect(() => {
    setLoading(true);
    setError(false);
    const url = "/api/jobs?profession=" + encodeURIComponent(profession || "software engineer") + "&location=" + encodeURIComponent(city.adzunaLocation) + "&country=" + city.adzunaCountry;
    fetch(url)
      .then(r => r.json())
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      {error && <p style={{ fontSize: 13, color: COLORS.textMuted }}>Could not load jobs. Try searching on LinkedIn or Indeed below.</p>}
...
function Dashboard({ profile, setPage }: { profile: FormState | null; setPage: (p: string) => void }) {
  ...
  const sendChat = async () => {
    ...
    setChatLoading(true);
    try {
      const context = "You are MoveAI's relocation assistant. The user is a " + profile?.profession + " with " + profile?.years + " years experience from " + profile?.country + " (city: " + profile?.city + "), speaking " + (profile?.languages || []).join(", ") + ". They prefer " + profile?.remote + " work and savings " + profile?.savings + ". They value: " + (profile?.lifestyle || []).join(", ") + ". Top matches: " + CITIES_DATA.slice(0, 3).map(c => c.city + " score:" + c.score + " salary:$" + c.salary + " rent:$" + c.rent).join("; ") + ". The dashboard shows live Adzuna job listings, flights, rentals and job board links for each city. Be concise and helpful.";
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ display: "inline-flex", gap: 8, alignItems: "center", background: "rgba(104,211,145,0.1)", border: "1px solid rgba(104,211,145,0.3)", borderRadius: 100, padding: "4px 14px", fontSize: 12, color: "#68D391", marginBottom: 12 }}>
              <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#68D391", display: "inline-block" }} /> AI Analysis Complete
            </div>
            ...
            <p style={{ color: COLORS.textSub, marginTop: 6, fontSize: 15 }}>Based on your {profile?.profession} profile • {profile?.years} years exp • from {profile?.country}</p>
          </div>
            ...
            <div key={city.city} onClick={() => setActiveCity(activeCity?.city === city.city ? null : city)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 180 }}>
                  ...
                  <div><div style={{ fontWeight: 700, fontSize: 18 }}>{city.city}</div><div style={{ fontSize: 13, color: COLORS.textMuted }}>{city.country}</div></div>
                </div>
