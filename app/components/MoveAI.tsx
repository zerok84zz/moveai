...
const COLORS = {
  bg: "#050814", bgCard: "rgba(255,255,255,0.04)",
  border: "rgba(255,255,255,0.08)", accent: "#63B3ED", accentAlt: "#9F7AEA",
  accentGreen: "#68D391", text: "#F7FAFC", textMuted: "#718096", textSub: "#A0AEC0",
};
...
interface CityData {
  ...
  adzunaLocation: string; adzunaCountry: string; jobCount?: number;
}

const DEMAND_COLOR: Record<string, string> = {
  "Very High": "#68D391", "High": "#63B3ED", "Medium": "#F6AD55", "Low": "#FC8181",
};
...
function GlowOrb({ x, y, color, size = 400 }: { x: string; y: string; color: string; size?: number }) {
  return <div style={{ position: "absolute", left: x, top: y, width: size, height: size, borderRadius: "50%", background: color, filter: "blur(120px)", opacity: 0.15, pointerEvents: "none", zIndex: 0 }} />;
}
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, borderBottom: "1px solid rgba(255,255,255,0.08)", background: "rgba(5,8,20,0.8)", backdropFilter: "blur(20px)", padding: "0 2rem", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        {["Features", "Pricing", "Blog"].map(item => (
          <button key={item} style={{ background: "none", border: "none", color: COLORS.textSub, fontSize: 14, padding: "6px 14px", cursor: "pointer", borderRadius: 6 }}
            onMouseOver={e => (e.target as HTMLButtonElement).style.color = COLORS.text}
            onMouseOut={e => (e.target as HTMLButtonElement).style.color = COLORS.textSub}>{item}</button>
        ))}
        <button onClick={() => setPage("onboarding")} style={{ background: "linear-gradient(135deg, #63B3ED, #9F7AEA)", border: "none", color: "#fff", fontSize: 14, fontWeight: 600, padding: "8px 20px", borderRadius: 8, cursor: "pointer" }}>Get Started</button>
      </div>
    </nav>
  );
}
...
function Landing({ setPage }: { setPage: (p: string) => void }) {
  ...
  useEffect(() => {
    const t = setInterval(() => { setFade(false); setTimeout(() => { setWordIdx(i => (i + 1) % words.length); setFade(true); }, 300); }, 2500);
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.text, fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ position: "relative", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px 2rem 4rem", textAlign: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0, backgroundImage: "linear-gradient(rgba(99,179,237,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(99,179,237,0.05) 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", background: "rgba(99,179,237,0.1)", border: "1px solid rgba(99,179,237,0.3)", borderRadius: 100, fontSize: 13, color: COLORS.accent, marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2.5rem, 6vw, 4.5rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-2px", marginBottom: 8 }}>Relocate and grow your career</h1>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2.5rem, 6vw, 4.5rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-2px", marginBottom: 32, background: "linear-gradient(135deg, #63B3ED, #9F7AEA, #68D391)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", opacity: fade ? 1 : 0, transition: "opacity 0.3s" }}>{words[wordIdx]}</h1>
          <p style={{ fontSize: "clamp(1rem, 2vw, 1.2rem)", color: COLORS.textSub, maxWidth: 560, margin: "0 auto 48px", lineHeight: 1.7 }}>MoveAI scans real job markets across 10+ cities in real time to find where your career will thrive most.</p>
        ...
        <div style={{ position: "relative", zIndex: 1, marginTop: 80, display: "flex", gap: 48, flexWrap: "wrap", justifyContent: "center" }}>
          {[["10+", "Cities Scanned Live"], ["Real-time", "Job Market Data"], ["94%", "Match Accuracy"], ["3min", "Analysis Time"]].map(([num, label]) => (
            <div key={label} style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 32, fontWeight: 700 }}>{num}</div>
      ...
      <section style={{ padding: "6rem 2rem", maxWidth: 1100, margin: "0 auto" }}>
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 4vw, 2.8rem)", fontWeight: 700, textAlign: "center", marginBottom: 64 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>
          {[
            { icon: "🤖", title: "Real-time AI Analysis", desc: "Our AI scans live job listings across 10+ cities the moment you submit your profile." },
            { icon: "✈️", title: "Flight Search", desc: "Direct links to Google Flights and Skyscanner with your route pre-filled." },
            { icon: "🏠", title: "Rental Finder", desc: "Browse real apartments on Spotahome and HousingAnywhere." },
            { icon: "💼", title: "Live Job Listings", desc: "Real job offers from Adzuna pulled in real time for your profession." },
            { icon: "💬", title: "AI Chat", desc: "Ask anything about cities, visas, salaries, and neighbourhoods." },
            { icon: "📊", title: "Dynamic Rankings", desc: "Cities ranked by actual job demand for your profession, not static data." },
          ].map(({ icon, title, desc }) => (
      ...
      <section style={{ padding: "6rem 2rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, marginBottom: 20 }}>Your next chapter starts here.</h2>
          <p style={{ color: COLORS.textSub, fontSize: 18, marginBottom: 40 }}>Free. 3 minutes. No credit card.</p>
          <button onClick={() => setPage("onboarding")} style={{ background: "linear-gradient(135deg, #63B3ED, #9F7AEA)", border: "none", color: "#fff", fontSize: 18, fontWeight: 700, padding: "18px 56px", borderRadius: 12, cursor: "pointer", boxShadow: "0 0 60px rgba(99,179,237,0.4)" }}>Start My Free Analysis</button>
        </div>
      </section>
...
function Onboarding({ setPage, setProfile }: { setPage: (p: string) => void; setProfile: (p: FormState) => void }) {
  ...
  const [form, setForm] = useState<FormState>({ profession: "", jobTitle: "", years: "3-5", languages: [], country: "", city: "", savings: "", monthlyBudget: "", preferredCountries: [], remote: "hybrid", lifestyle: [], reloPriority: "quality" });
  const update = (k: keyof FormState, v: string) => setForm(f => ({ ...f, [k]: v }));
  const toggleArr = (k: keyof FormState, v: string) => setForm(f => { const arr = f[k] as string[]; return { ...f, [k]: arr.includes(v) ? arr.filter((x: string) => x !== v) : [...arr, v] }; });
  const canNext = () => { if (step === 0) return form.profession; if (step === 1) return form.years && form.languages.length > 0; if (step === 2) return form.country; if (step === 3) return form.savings; return true; };
  const handleNext = () => { if (step < STEPS.length - 1) setStep(s => s + 1); else { setProfile(form); setPage("dashboard"); } };
  const stepContent = [
    <div key="s0">
      <label style={labelStyle}>What is your field?</label>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 24 }}>
        {["Software Engineering", "Data Science", "Product Management", "Design", "Marketing", "Finance", "Healthcare", "Legal", "Education"].map(p => <button key={p} onClick={() => update("profession", p)} style={chipStyle(form.profession === p)}>{p}</button>)}
      </div>
    ...
    <div key="s1">
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        {["0-1", "1-3", "3-5", "5-10", "10+"].map(y => <button key={y} onClick={() => update("years", y)} style={chipStyle(form.years === y)}>{y} yrs</button>)}
      </div>
      <label style={labelStyle}>Languages spoken</label>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {["English", "Spanish", "French", "German", "Portuguese", "Mandarin", "Arabic", "Japanese", "Other"].map(l => <button key={l} onClick={() => toggleArr("languages", l)} style={chipStyle(form.languages.includes(l))}>{l}</button>)}
      </div>
    </div>,
    <div key="s2">
      <label style={labelStyle}>Current country</label>
      <input value={form.country} onChange={e => update("country", e.target.value)} placeholder="e.g. Spain, Brazil, USA..." style={inputStyle} />
      <label style={labelStyle}>Current city (used for flight search)</label>
      <input value={form.city} onChange={e => update("city", e.target.value)} placeholder="e.g. Madrid, London, NYC..." style={{ ...inputStyle, marginBottom: 0 }} />
    </div>,
    <div key="s3">
      <label style={labelStyle}>Current savings (USD)</label>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        {["<$5K", "$5K-$15K", "$15K-$30K", "$30K-$60K", "$60K+"].map(s => <button key={s} onClick={() => update("savings", s)} style={chipStyle(form.savings === s)}>{s}</button>)}
      </div>
      <label style={labelStyle}>Monthly budget for living (USD)</label>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {["<$1.5K", "$1.5K-$2.5K", "$2.5K-$4K", "$4K-$6K", "$6K+"].map(b => <button key={b} onClick={() => update("monthlyBudget", b)} style={chipStyle(form.monthlyBudget === b)}>{b}</button>)}
      </div>
    </div>,
    <div key="s4">
      <label style={labelStyle}>Work arrangement</label>
      <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
        {[["remote", "Remote"], ["hybrid", "Hybrid"], ["onsite", "On-site"]].map(([v, l]) => <button key={v} onClick={() => update("remote", v)} style={{ ...chipStyle(form.remote === v), flex: 1 }}>{l}</button>)}
      </div>
      <label style={labelStyle}>Lifestyle priorities</label>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        {["Nightlife", "Nature", "Safety", "Diversity", "Affordable", "Weather", "Culture", "Beach", "Food scene"].map(l => <button key={l} onClick={() => toggleArr("lifestyle", l)} style={chipStyle(form.lifestyle.includes(l))}>{l}</button>)}
      </div>
      <label style={labelStyle}>Preferred regions</label>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {["Western Europe", "Eastern Europe", "North America", "Latin America", "Southeast Asia", "Middle East", "Oceania"].map(r => <button key={r} onClick={() => toggleArr("preferredCountries", r)} style={chipStyle(form.preferredCountries.includes(r))}>{r}</button>)}
      </div>
    </div>,
  ];
  return (
      ...
      <div style={{ width: "100%", maxWidth: 560, position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            {STEPS.map((s, i) => <div key={s.id} style={{ width: "18%", height: 3, borderRadius: 2, background: i <= step ? "linear-gradient(90deg, #63B3ED, #9F7AEA)" : "rgba(255,255,255,0.1)", transition: "background 0.4s" }} />)}
          </div>
...
function JobListings({ city, profession }: { city: CityData; profession: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/jobs?profession=" + encodeURIComponent(profession || "software engineer") + "&location=" + encodeURIComponent(city.adzunaLocation) + "&country=" + city.adzunaCountry)
      .then(r => r.json())
      .then(data => { setJobs(data.results || []); setLoading(false); })
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      {loading && <div style={{ display: "flex", gap: 6, alignItems: "center", padding: 16, background: "rgba(255,255,255,0.03)", borderRadius: 10 }}>{[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: COLORS.accent, animation: "bounce 1s " + i*0.2 + "s infinite" }} />)}<span style={{ fontSize: 13, color: COLORS.textMuted, marginLeft: 8 }}>Fetching live jobs...</span></div>}
      {error && <p style={{ fontSize: 13, color: COLORS.textMuted }}>Could not load jobs. Try LinkedIn or Indeed below.</p>}
      {!loading && !error && jobs.length === 0 && <p style={{ fontSize: 13, color: COLORS.textMuted }}>No listings found right now. Try LinkedIn or Indeed below.</p>}
...
function Dashboard({ profile, setPage }: { profile: FormState | null; setPage: (p: string) => void }) {
  ...
  const [chatMessages, setChatMessages] = useState([{ role: "ai", text: "Hi! I am scanning live job markets across 10+ cities for your profession. This takes about 20 seconds..." }]);
  ...
  useEffect(() => {
    fetch("/api/analyze", {
      ...
      body: JSON.stringify({ profession: profile?.profession || "software engineer" })
    })
      .then(r => r.json())
      .then(data => {
        setCities(data.cities || []);
        setAnalyzing(false);
        setChatMessages([{ role: "ai", text: "Analysis complete! I scanned live job listings across 10 cities for " + (profile?.profession || "your profession") + ". Click any city to see real opportunities, flights and rentals!" }]);
      })
  ...
  const sendChat = async () => {
    ...
    setChatLoading(true);
    try {
      const context = "You are MoveAI relocation assistant. User is a " + profile?.profession + " with " + profile?.years + " years from " + profile?.country + " (" + profile?.city + "). Top cities from live scan: " + cities.slice(0,3).map(c => c.city + " score:" + c.score + " jobs:" + (c.jobCount || 0)).join(", ") + ". Be concise and helpful.";
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 800, system: context, messages: [...chatMessages.filter((m,idx) => m.role !== "ai" || idx > 0).map(m => ({ role: m.role === "ai" ? "assistant" : "user", content: m.text })), { role: "user", content: userMsg }] }) });
      const data = await res.json();
      setChatMessages(m => [...m, { role: "ai", text: data.content?.map((c: {text?:string}) => c.text||"").join("") || "I am here to help!" }]);
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bg, fontFamily: "'DM Sans', sans-serif", color: COLORS.text, paddingTop: 64 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2.5rem 1.5rem 4rem" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ display: "inline-flex", gap: 8, alignItems: "center", background: analyzing ? "rgba(99,179,237,0.1)" : "rgba(104,211,145,0.1)", border: "1px solid " + (analyzing ? "rgba(99,179,237,0.3)" : "rgba(104,211,145,0.3)"), borderRadius: 100, padding: "4px 14px", fontSize: 12, color: analyzing ? COLORS.accent : "#68D391", marginBottom: 12 }}>
              <span style={{ width: 5, height: 5, borderRadius: "50%", background: analyzing ? COLORS.accent : "#68D391", display: "inline-block", animation: analyzing ? "pulse 1s infinite" : "none" }} />
              {analyzing ? "Scanning live job markets..." : "AI Analysis Complete"}
            </div>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.6rem, 3vw, 2.4rem)", fontWeight: 700, letterSpacing: "-0.5px", margin: 0 }}>
              {analyzing ? "Analyzing 10+ cities in real time..." : "Your Top City Matches"}
            </h1>
            <p style={{ color: COLORS.textSub, marginTop: 6, fontSize: 15 }}>
              {analyzing ? "Scanning live job demand for " + profile?.profession + "..." : "Ranked by real job demand for " + profile?.profession + " from " + profile?.country}
            </p>
          </div>
          <button onClick={() => setChatOpen(o => !o)} style={{ display: "flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg, rgba(99,179,237,0.15), rgba(159,122,234,0.15))", border: "1px solid rgba(99,179,237,0.3)", borderRadius: 10, padding: "10px 20px", color: COLORS.accent, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>💬 AI Assistant</button>
        </div>

        {analyzing ? (
          <div style={{ display: "grid", gap: 16 }}>
            {[1,2,3,4,5,6].map(i => (
              <div key={i} style={{ background: COLORS.bgCard, border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "24px", animation: "pulse 2s " + i*0.2 + "s infinite" }}>
                <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.06)" }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ width: "30%", height: 16, background: "rgba(255,255,255,0.06)", borderRadius: 4, marginBottom: 8 }} />
                    <div style={{ width: "20%", height: 12, background: "rgba(255,255,255,0.04)", borderRadius: 4 }} />
                  </div>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 24 }}>
              {[
                { label: "Cities Scanned", value: "10+", icon: "🌍" },
                { label: "Best Match Score", value: cities[0]?.score + "/100" || "—", icon: "⭐" },
                { label: "Top City", value: cities[0]?.city || "—", icon: "🏆" },
                { label: "Live Jobs Found", value: cities.reduce((a,c) => a + (c.jobCount||0), 0) + "+", icon: "💼" },
            ...
            <div style={{ display: "grid", gap: 16 }}>
              {cities.map((city, i) => (
                <div key={city.city} onClick={() => setActiveCity(activeCity?.city === city.city ? null : city)}
                  style={{ background: activeCity?.city === city.city ? "rgba(99,179,237,0.07)" : COLORS.bgCard, border: "1px solid " + (activeCity?.city === city.city ? "rgba(99,179,237,0.4)" : "rgba(255,255,255,0.08)"), borderRadius: 16, padding: "20px 24px", cursor: "pointer", transition: "all 0.3s" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: 24, flexWrap: "wrap", flex: 1, justifyContent: "flex-end" }}>
                      {[
                        ...
                        { label: "Live Jobs", value: (city.jobCount||0) + " found", highlight: true },
                    ...
                    <div style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 20 }}>
                        {[
                          ...
                          { label: "Live Jobs Found", value: (city.jobCount||0).toString(), color: COLORS.accentGreen },
                      ...
                      <div style={{ background: "rgba(99,179,237,0.06)", border: "1px solid rgba(99,179,237,0.15)", borderRadius: 10, padding: "14px 18px", marginBottom: 4 }}>
      ...
      <style>{`@keyframes bounce{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-8px)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
    </div>
  );
}
